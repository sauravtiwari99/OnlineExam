...
export class TestLevel2Component implements OnInit {
  testName
  showTest = true;
  level2;
  selectedOptions=[]
  ...
  this.service.getLevelTwo({ "subject" : this.testName,"set" : "1"}).subscribe(
    data=>{
      this.level2 = data;
    });
  }
...
submitTest(){
  for(var i =0; i < this.level2.length ; i++){
    if(this.selectedOptions[i]==this.level2[i].correct_option){
      this.totalMarks +=2;
    }
  }
  ...
  this.service.storeLevelTwo({"report_id":sessionStorage.getItem("report_id"),"level2_score":this.totalMarks}).subscribe(
    data=>{
